<!-- 
만든이: 2017038023 반예린
해당 소스파일 정보: 게시글 삭제 폼 관리자버전.
			 관리자용 게시글 삭제 폼 출력 -> 게시글 삭제를 위한 관리자 비밀번호 확인 폼을 출력함.
			 관리자 비밀번호를 입력 후 '게시글 삭제'버튼을 클릭하면 실제로 삭제 과정을 진행하는 ReviewRemoveActionManager.jsp로 이동함.
 -->
...
%>
    <form action="<%=request.getContextPath()%>/Yerin/ReviewRemoveActionManager.jsp" method="post">
        <input name="boardNo" value="<%=request.getParameter("boardNo")%>" type="hidden"/>
        <div>비밀번호확인(관리자 비밀번호) :</div>
