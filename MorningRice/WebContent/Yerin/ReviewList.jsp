...
해당 소스파일 정보: 게시글 목록 출력 기능.
			 ...
			 게시글의 제목을 클릭하면 해당 게시글의 내용을 자세히 볼 수 있는 ReviewView.jsp로 이동함.
 -->
...
try {
    ...
    String totalSql = "SELECT COUNT(*) FROM review_board";
...
<%    
    ...
    String aiSortSql = "ALTER TABLE review_board AUTO_INCREMENT=1; SET @COUNT = 0; UPDATE review_board SET review_board.board_no = @COUNT:=@COUNT+1;";
    aiSortStmt = connection.createStatement();
    aiSortStmt.executeUpdate(aiSortSql);
    
  	//게시글 레코드들을 게시글 번호의 오름차순으로 추출하여 결과객체에 넣음
    String listSql ="SELECT board_no, board_title, board_user, board_date FROM review_board ORDER BY board_no";
...
%>
<div class="container">
<h2 style="font-weight: bold;">이용후기</h2>
  <p>고객님들의 소중한 이용후기가 담긴 게시판입니다.</p>
...
%>
                <tr>
                    ...
                    <td>
						<a href="<%=request.getContextPath()%>/Yerin/ReviewView.jsp?boardNo=<%=listResultSet.getInt("board_no")%>">
...
%>
    ...
    <hr />
    <a href="<%=request.getContextPath()%>/Yerin/ReviewAddForm.jsp" class="btn btn-primary float-right">게시글 입력</a>
...
%>
            <a href="<%=request.getContextPath()%>/Yerin/ReviewList.jsp?currentPage=<%=currentPage-1%>">이전</a>
<%
        }
...
%>
            ...
            <a href="<%=request.getContextPath()%>/Yerin/ReviewList.jsp?currentPage=<%=currentPage+1%>">다음</a>
<%
        }
%>
    </div>
