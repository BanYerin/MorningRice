...
if(request.getParameter("boardNo") == null) {//게시글번호가 null일 경우
	...
	out.flush();//버퍼 비워줌
	response.sendRedirect(request.getContextPath()+"/Yerin/Q&AList.jsp"); //게시글 목록으로 이동
    ...
    try {
        ...
        String sql = "SELECT board_title, board_content, board_user, board_date FROM qa_board WHERE board_no=?";
	...
	<table border="1" cellspacing="1" cellpadding="5" bgcolor="#E6D7FA">
            ...
            <tr>
     		   <td colspan="2" align="center">
     		   	<a href="<%=request.getContextPath()%>/Yerin/Q&AModifyForm.jsp?boardNo=<%=boardNo%>" class="btn btn-primary float-left">수정</a> 
     		   	<a href="<%=request.getContextPath()%>/Yerin/Q&ARemoveForm.jsp?boardNo=<%=boardNo%>" class="btn btn-primary float-left">삭제</a>
     		    <a href="<%=request.getContextPath()%>/Yerin/Q&AList.jsp?boardNo=<%=boardNo%>" class="btn btn-primary float-right">목록</a>
     		   </td>
   			</tr>

	</table>
	<a href="<%=request.getContextPath()%>/Yerin/Q&ARemoveFormManager.jsp?boardNo=<%=boardNo%>" style="color:red; text-decoration:underline">관리자 권한으로 삭제</a>
</div>
