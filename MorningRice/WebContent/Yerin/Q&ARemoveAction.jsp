...
if(request.getParameter("boardNo") == null || request.getParameter("boardPw") == null) {//게시글번호가 null이거나 게시글 삭제폼에 입력받은 비밀번호가 null일 경우
    ...
    try {
        ...
        System.out.println("DB연결완료");
     	...
     	//현재 삭제하려는 게시글의 게시글번호에 해당하는 레코드를 qa_board로부터 추출하여 board_pw를 가져와 DBpw에 넣음
		stmt = connection.createStatement();
		rs=stmt.executeQuery("SELECT * FROM qa_board WHERE board_no="+boardNo);
        ...
        String sql = "DELETE FROM qa_board WHERE board_no=? AND board_pw=?";
        ...
        if(statement.executeUpdate()==1){//삭제명령 실행 결과가 참이면
            ...
            response.sendRedirect(request.getContextPath()+"/Yerin/Q&ARemoveForm.jsp?boardNo="+boardNo);
       	 	}
